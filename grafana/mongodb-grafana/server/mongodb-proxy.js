...
var serverConfig = {
	...
	logRequests: process.env.LOGREQUESTS || false,
	logQueries: process.env.LOGQUERIES || false,
	logTimings: process.env.LOGTIMINGS || false
}
...
function getTableResults(docs)
{
  ...
  // If columns empty we will add one to avoid find error in grafana
  if(Object.values(columns).length == 0){
  	columns["value"] = {
          text: "value"
    }
    rows = [[null]]
