...
help()  {
  ...
  echo "sidechainandbridgesnonreg.sh OPTIONS
Usage:
REQUIRED:
...
./topUpWallets --base=admin --from=1 --to=${NB_WALLETS} --minETH=${ETH_AMOUNT} --maxETH=${ETH_AMOUNT} --chain=dev --minRLC=${RLC_AMOUNT} --keystoredir=/home/ubuntu/wallets/wallets
...
cp -rf ${SCRIPT_DIR}/poa-bridge-contracts-sidechain-dev.env ${SCRIPT_DIR}/poa-bridge-contracts-sidechain-dev.env.ori
sed -i "s/__ADMIN_WALLET_PRIVATEKEY__/${ADMIN_PRIVATE_KEY}/g" ${SCRIPT_DIR}/poa-bridge-contracts-sidechain-dev.env
sed -i "s/__ADMIN_WALLET__/${ADMIN_ADDRESS}/g" ${SCRIPT_DIR}/poa-bridge-contracts-sidechain-dev.env
sed -i "s/__ERC20_TOKEN_ADDRESS__/${RlcAddress}/g" ${SCRIPT_DIR}/poa-bridge-contracts-sidechain-dev.env
sed -i "s/__HOME_RPC_URL__/http:\/\/${TEST_IP}:8545/g" ${SCRIPT_DIR}/poa-bridge-contracts-sidechain-dev.env
sed -i "s/__FOREIGN_RPC_URL__/http:\/\/${TEST_IP}:9545/g" ${SCRIPT_DIR}/poa-bridge-contracts-sidechain-dev.env



cp ${SCRIPT_DIR}/poa-bridge-contracts-sidechain-dev.env deploy/.env
